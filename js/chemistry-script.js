...
function initAtomicStructureSimulator() {
    const container = document.getElementById('atoms-periodic-table');
    if (!container) return;
    ...
    function drawAtom(element) {
        ...
        ctx.fillStyle = 'black';
        ctx.font = '16px Arial';
        ctx.fillText(`${element.name} (${element.symbol})`, 10, 20);
        ctx.fillText(`Protons: ${element.protons}`, 10, 40);
        ctx.fillText(`Neutrons: ${element.neutrons}`, 10, 60);
        ctx.fillText(`Electrons: ${element.electrons}`, 10, 80);
    }

    select.addEventListener('change', (e) => {
        const selectedElement = elements.find(el => el.symbol === e.target.value);
        drawAtom(selectedElement);
    });
...
function initMoleCalculator() {
    const container = document.getElementById('mole-ram');
    if (!container) return;

    const input = document.createElement('input');
    input.type = 'number';
    input.placeholder = 'Enter mass (g)';
    container.appendChild(input);

    const select = document.createElement('select');
    const elements = [
        { name: 'Hydrogen', mass: 1.008 },
        { name: 'Carbon', mass: 12.011 },
        { name: 'Oxygen', mass: 15.999 },
        { name: 'Sodium', mass: 22.990 },
        { name: 'Chlorine', mass: 35.45 }
    ];
    elements.forEach(element => {
        const option = document.createElement('option');
        option.value = element.mass;
        option.textContent = element.name;
        select.appendChild(option);
    });
    container.appendChild(select);
    ...
    function calculateMoles() {
        ...
        const atomicMass = parseFloat(select.value);
        if (mass && atomicMass) {
    ...
    input.addEventListener('input', calculateMoles);
    select.addEventListener('change', calculateMoles);
}

function initAvogadroSimulator() {
    const container = document.getElementById('avogadro-number');
    if (!container) return;

    const input = document.createElement('input');
    input.type = 'number';
    input.placeholder = 'Enter number of moles';
    container.appendChild(input);
    ...
    function calculateParticles() {
        const moles = parseFloat(input.value);
        if (moles) {
            const particles = moles * 6.022e23;
            result.textContent = `Number of particles: ${particles.toExponential(4)}`;
            drawParticles(moles);
        } else {
            result.textContent = 'Please enter a valid number of moles';
            ctx.clearRect(0, 0, canvas.width, canvas.height);
        }
    }

    function drawParticles(moles) {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        const maxParticles = 1000;
        const particlesToDraw = Math.min(moles * 100, maxParticles);
    ...
    input.addEventListener('input', calculateParticles);
}

function initGasVolumeCalculator() {
    const container = document.getElementById('molar-gas-volume');
    if (!container) return;
    ...
    function calculateVolume() {
        ...
        if (moles) {
            ...
            result.textContent = `Volume at STP: ${volume.toFixed(2)} L`;
        } else {
            result.textContent = 'Please enter a valid number of moles';
        }
    }
...
function initEmpiricalFormulaCalculator() {
    const container = document.getElementById('empirical-molecular');
    if (!container) return;

    const elementInputs = document.createElement('div');
    elementInputs.innerHTML = `
        <input type="text" placeholder="Element symbol" class="element-symbol">
        <input type="number" placeholder="Mass (g)" class="element-mass">
    `;
    container.appendChild(elementInputs);

    const addElementBtn = document.createElement('button');
    addElementBtn.textContent = 'Add Element';
    container.appendChild(addElementBtn);

    const calculateBtn = document.createElement('button');
    calculateBtn.textContent = 'Calculate Empirical Formula';
    ...
    addElementBtn.addEventListener('click', () => {
        const newInputs = elementInputs.cloneNode(true);
        container.insertBefore(newInputs, addElementBtn);
    });

    calculateBtn.addEventListener('click', () => {
        const elements = [];
        const symbolInputs = container.querySelectorAll('.element-symbol');
        const massInputs = container.querySelectorAll('.element-mass');
        
        for (let i = 0; i < symbolInputs.length; i++) {
            const symbol = symbolInputs[i].value;
            const mass = parseFloat(massInputs[i].value);
            if (symbol && mass) {
        ...
        if (elements.length < 2) {
            result.textContent = 'Please enter at least two elements';
            return;
        }

        // Simple empirical formula calculation (not accounting for atomic masses)
        const minMass = Math.min(...elements.map(e => e.mass));
        const formula = elements.map(e => {
            const ratio = Math.round(e.mass / minMass);
            return `${e.symbol}${ratio > 1 ? ratio : ''}`;
        }).join('');

        result.textContent = `Empirical Formula: ${formula}`;
    });
}

function initPercentCompositionCalculator() {
    const container = document.getElementById('percent-composition');
    if (!container) return;

    const elementInputs = document.createElement('div');
    elementInputs.innerHTML = `
        <input type="text" placeholder="Element symbol" class="element-symbol">
        <input type="number" placeholder="Atomic mass" class="element-mass">
        <input type="number" placeholder="Number of atoms" class="element-count">
    `;
    container.appendChild(elementInputs);
    ...
    const calculateBtn = document.createElement('button');
    calculateBtn.textContent = 'Calculate Percent Composition';
    container.appendChild(calculateBtn);
    ...
    calculateBtn.addEventListener('click', () => {
        ...
        const symbolInputs = container.querySelectorAll('.element-symbol');
        const massInputs = container.querySelectorAll('.element-mass');
        const countInputs = container.querySelectorAll('.element-count');
        
        for (let i = 0; i < symbolInputs.length; i++) {
            const symbol = symbolInputs[i].value;
            const mass = parseFloat(massInputs[i].value);
            const count = parseInt(countInputs[i].value);
            if (symbol && mass && count) {
                elements.push({ symbol, mass, count });
            }
        }

        if (elements.length < 1) {
            result.textContent = 'Please enter at least one element';
            return;
        }

        const totalMass = elements.reduce((sum, e) => sum + e.mass * e.count, 0);
        const percentages = elements.map(e => {
            const percent = (e.mass * e.count / totalMass) * 100;
            return `${e.symbol}: ${percent.toFixed(2)}%`;
        });

        result.textContent = `Percent Composition: ${percentages.join(', ')}`;
    });
}

function initChemicalReactionSimulator() {
    const container = document.getElementById('chemical-reactions');
    if (!container) return;
    ...
    const reactions = [
        { name: 'Exothermic', color: 'red', direction: -1 },
        { name: 'Endothermic', color: 'blue', direction: 1 },
        { name: 'Neutral', color: 'green', direction: 0 }
    ];
    reactions.forEach(reaction => {
        const option = document.createElement('option');
        option.value = reaction.name;
        option.textContent = reaction.name;
        reactionSelect.appendChild(option);
    });
    container.appendChild(reactionSelect);
    ...
    function drawReaction(reaction) {
        ...
        ctx.moveTo(50, 100);
        ctx.lineTo(100, 100);
        ctx.lineTo(150, 100 + reaction.direction * 50);
        ctx.lineTo(250, 100 + reaction.direction * 50);
        ...
        ctx.fillStyle = 'black';
        ctx.font = '14px Arial';
        ctx.fillText('Reactants', 20, 90);
        ctx.fillText('Products', 260, 90);
        ctx.fillText('Energy', 10, 20);

        if (reaction.direction !== 0) {
            ctx.fillText(reaction.direction > 0 ? 'Energy absorbed' : 'Energy released', 100, 30);
        }
    }

    reactionSelect.addEventListener('change', (e) => {
        const selectedReaction = reactions.find(r => r.name === e.target.value);
        drawReaction(selectedReaction);
    });

    // Initial draw
    drawReaction(reactions[0]);
}

function initEquilibriumSimulator() {
    const container = document.getElementById('reversible-reactions');
    if (!container) return;
    ...
    function drawEquilibrium(position) {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ...
        const reactantWidth = position * 2;
        const productWidth = 200 - reactantWidth;

        ctx.fillStyle = 'blue';
        ctx.fillRect(50, 50, reactantWidth, 100);

        ctx.fillStyle = 'red';
        ctx.fillRect(50 + reactantWidth, 50, productWidth, 100);
        ...
        ctx.fillStyle = 'black';
        ctx.font = '14px Arial';
        ctx.fillText('Reactants', 60, 40);
        ctx.fillText('Products', 200, 40);
    }

    slider.addEventListener('input', (e) => {
        const position = parseInt(e.target.value);
        drawEquilibrium(position);
    });
...
function initLeChatelierSimulator() {
    const container = document.getElementById('effect-conditions');
    if (!container) return;
    ...
    const conditions = ['Increase Temperature', 'Decrease Temperature', 'Increase Pressure', 'Decrease Pressure', 'Add Reactant', 'Remove Product'];
    ...
    function drawLeChatelier(condition) {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ...
        ctx.moveTo(150, 50);
        let arrowEnd = 150;
        if (condition.includes('Increase') || condition === 'Add Reactant') {
            arrowEnd = 200;
        } else if (condition.includes('Decrease') || condition === 'Remove Product') {
            arrowEnd = 100;
        }
        ctx.lineTo(arrowEnd, 50);
        ctx.strokeStyle = 'red';
        ctx.stroke();
        ...
        ctx.fillText('Reactants', 20, 90);
        ctx.fillText('Products', 260, 90);
        ctx.fillText(condition, 10, 30);
    }

    conditionSelect.addEventListener('change', (e) => {
        drawLeChatelier(e.target.value);
    });

    // Initial draw
    drawLeChatelier(conditions[0]);
}

function initAcidCarbonateSimulator() {
    const container = document.getElementById('acid-carbonate');
    if (!container) return;
    ...
    function drawReaction() {
        ...
        animationId = requestAnimationFrame(drawReaction);
    }
...
function initQuiz() {
    ...
    container.id = 'quiz';
    container.innerHTML = `
        <h2>Chemistry Knowledge Check</h2>
    ...
    const quizQuestions = [
        ...
        {
            question: "How many grams of hydrogen are in 1 mole of H2O?",
            options: ["1 g", "2 g", "16 g", "18 g"],
            correctAnswer: 1,
            explanation: "In 1 mole of H2O, there are 2 moles of hydrogen atoms. Each hydrogen atom has a mass of approximately 1 g/mol, so 2 moles of hydrogen would have a mass of 2 g."
        },
        {
        ...
        },
        {
            ...
            options: ["22.4 mL", "22.4 L", "1 L", "100 L"],
            correctAnswer: 1,
            explanation: "At standard temperature and pressure (STP), 1 mole of an ideal gas occupies a volume of 22.4 liters."
        },
        {
            question: "Which of the following is an empirical formula?",
            options: ["H2O", "CH2O", "C6H12O6", "NaCl"],
            correctAnswer: 1,
            explanation: "CH2O is an empirical formula as it represents the simplest whole number ratio of atoms in the compound."
        },
        {
            question: "What is the percent composition of oxygen in H2O?",
            options: ["88.9%", "11.1%", "16.7%", "83.3%"],
            correctAnswer: 3,
            explanation: "In H2O, oxygen has a mass of 16 g/mol out of a total molar mass of 18 g/mol. (16/18) * 100 = 88.9%"
        },
        {
        ...
        {
            question: "What does chemical equilibrium mean?",
            options: ["Reactions stop completely", "Forward reaction rate equals reverse reaction rate", "All reactants are converted to products", "Temperature remains constant"],
            correctAnswer: 1,
            explanation: "Chemical equilibrium is reached when the rate of the forward reaction equals the rate of the reverse reaction."
        },
        {
            question: "According to Le Chatelier's principle, what happens when pressure is increased in a gaseous equilibrium system?",
            options: ["Equilibrium always shifts to reactants", "Equilibrium always shifts to products", "Equilibrium shifts to the side with fewer gas molecules", "Pressure has no effect on gaseous equilibrium"],
            correctAnswer: 2,
            explanation: "Le Chatelier's principle states that when pressure is increased, the equilibrium shifts to the side with fewer gas molecules to counteract the change."
        },
        {
            question: "What gas is produced in the reaction between hydrochloric acid and sodium carbonate?",
            options: ["Hydrogen", "Oxygen", "Carbon dioxide", "Chlorine"],
            correctAnswer: 2,
            explanation: "The reaction between hydrochloric acid and sodium carbonate produces carbon dioxide gas, along with water and sodium chloride."
        }
    ];
        ...
        quizQuestions.forEach((q, index) => {
            ...
            if (selectedAnswer) {
                ...
                if (userAnswer === q.correctAnswer) {
                    score++;
                    feedback += `<p>Question ${index + 1}: Correct! ${q.explanation}</p>`;
                } else {
                    feedback += `<p>Question ${index + 1}: Incorrect. ${q.explanation}</p>`;
                }
            } else {
                feedback += `<p>Question ${index + 1}: Not answered. ${q.explanation}</p>`;
            }
        });
