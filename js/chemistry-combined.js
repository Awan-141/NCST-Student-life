...
const elements = [
  ...
  elements.forEach(element => {
    const material = new THREE.MeshBasicMaterial({ color: getCategoryColor(element.category) });
    const cube = new THREE.Mesh(geometry, material);
    cube.position.set(element.x - 8.5, -element.y + 4, element.z);
    cube.userData = element;
    group.add(cube);
    cubes.push(cube);
  
    // Add text sprite for element symbol
    const sprite = makeTextSprite(element.symbol);
    sprite.position.set(element.x - 8.5, -element.y + 4, element.z + 0.5);
    group.add(sprite);
  });
  ...
  function getCategoryColor(category) {
    const colors = {
      ...
      'transition-metal': 0x54a0ff,
      'post-transition': 0x5f27cd,
      'metalloid': 0x48dbfb,
      'nonmetal': 0x1dd1a1,
      'noble gas': 0xff9ff3,
      'lanthanide': 0x00d2d3,
    ...
    return colors[category.toLowerCase()] || 0xcccccc; // ```javascript
  }
  ...
  // Show element info in the panel
